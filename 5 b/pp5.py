...
def generar_clave_y_comenzar():
    # Obtener los parámetros de la curva
    a = entry_a.get()
    b = entry_b.get()
    p = entry_p.get()
    
    # Generar las claves para el usuario actual
    private_key, public_key = generar_claves(a, b, p)
...
def cargar_claves_usuarios():
    ...
    # Cargar los parámetros de la curva del primer usuario
    a = entry_a.get()
    b = entry_b.get()
    p = entry_p.get()
...
tk.Label(frame_parametros, text="Parámetros de la Curva Elíptica").grid(row=0, column=0, columnspan=2)

tk.Label(frame_parametros, text="a:").grid(row=1, column=0)
entry_a = tk.Entry(frame_parametros)
entry_a.grid(row=1, column=1)

tk.Label(frame_parametros, text="b:").grid(row=2, column=0)
entry_b = tk.Entry(frame_parametros)
entry_b.grid(row=2, column=1)

tk.Label(frame_parametros, text="p:").grid(row=3, column=0)
entry_p = tk.Entry(frame_parametros)
